...
def sample(histogram):
  ...
  for key, value in histogram.items():
    accumulator += value/total # adds the weighted value of the word
...
def test_sample(histogram, loops):
  ...
  test_string = ''
  for i in range(loops): # run sample loop number of times
  ...
  weighted_histogram = {} 
  for key, value in test_histogram.items(): # find the percentage each word occurs and add it to weighted_histogram
    weighted_histogram[key] = value/total
